...
class TestClassLoader extends PHPUnit_Framework_TestCase {
    ...
    public function setUp() {
        ...
        ini_set('include_path', getcwd() . '/files' . ':' . $this->previousPath);
    }
    ...
    /**
     * Tests the throwing of exception when class not found.
     */
    public function testLoadMissingClass() {

        try {
            $test = new Controller_Test();
        }
        catch (ClassNotFoundException $ex) {
            return;
        }

        $this->fail('ClassNotFoundException expected exception has not been raised.');
    }

    /**
     * Tests the direct loading of class using the class name specified in dot notation.
     */
    public function testLoadClassDotForm() {


        $test = ClassLoader::loadClass('example.Controller.Test1');
        if ($test) {
            $class = new Test1();
            $this->assertEquals('Test1', $class->test);
            $this->assertTrue(method_exists($class, 'testFunc'));
        }
        else {
            $this->fail('Class example.Controller.Test1 was not loaded.');
        }
    }

    /**
     * Tests the direct loading of class using the class name specified in
     * underscore notation.
     */
    public function testLoadClassUnderscoreForm() {


        $test = ClassLoader::loadClass('example_Controller_Test');
        if ($test) {
            $class = new example_Controller_Test();
            $this->assertEquals('Test', $class->test);
            $this->assertTrue(method_exists($class, 'processRequest'));
        }
        else {
            $this->fail('Class example_Controller_Test was not loaded.');
        }
    }
